...
public:
	...
	SkyDome(HWND hWnd, OpenGLClass* OpenGL, int stacks, int slices, float radio, const wchar_t nombre[], const wchar_t nombre2[], const wchar_t nombre3[], int numtext, int numtext2, int numtext3)
	{
		this->hWnd = hWnd;
		sale = OpenGL;
		...
		CargarTextura(OpenGL, nombre, numtext);
		CargarTextura(OpenGL, nombre2, numtext2);
		CargarTextura(OpenGL, nombre3, numtext3);
	...
	void CargarTextura(OpenGLClass* OpenGL, const wchar_t nombre[], int numtxet) {
		//cargamos la textura de la figura
		Carga(nombre);
		int an = Ancho();
		int alt = Alto();
		// Set the unique texture unit in which to store the data.
		OpenGL->glActiveTexture(GL_TEXTURE0 + numtxet);
