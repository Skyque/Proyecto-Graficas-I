...
void CameraClass::Render(float vel, bool Chocando, float X, float Z, float Radio)
{
	...
	if (sqrt(pow(m_positionX, 2) + pow(m_positionZ, 2)) >= 230 || (Chocando && sqrt(pow(m_positionX - X, 2) + pow(m_positionZ - Z, 2)) <= Radio)) {
		m_positionX = PosAux1;
		m_positionZ = PosAux2;
	}
