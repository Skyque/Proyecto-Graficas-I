...
public:
	...
	VectorRR Up, derecha;
	float x, y, z, escala, RadioColisión;
	...
	Modelos(HWND hWnd, OpenGLClass* OpenGL, const char Modelo[], const wchar_t textura[], const wchar_t normales[], const wchar_t especular[], float escala, float x, float z, float Radio,
		int numtext, int numtext2, int numtext3)
	{
		this->escala = escala;
		this->x = x;
		this->y = y;
		this->z = z;
		this->RadioColisión = Radio * escala;
		sale = OpenGL;
		Up.X = 0;
		Up.Y = 1;
		Up.Z = 0;
		...
		CargarTextura(normales, numtext2, OpenGL);
		CargarTextura(especular, numtext3, OpenGL);
	...
	bool modeLoad(const char Modelo[])
	{
		struct vector3 {
			float x, y, z;
		};
